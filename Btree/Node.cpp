...
Node::Node(int order, bool leaf)    //Создаем узел с размером и указанием листовой ли узел 
{
	...
	childrens[order] = NULL;
	this->order = order;
	this->leaf = true;
	keys_count = 0;
}

void Node::add_key(int key, int index)
{
	if (index < keys_count) {
		keys[index] = key;
		keys_count++;
...
void Node::add_children(pointer key, int index)
{
	if (index <= keys_count) {
		childrens[index] = key;
		if (childrens) {
...
void Node::Traverse() {      //Обход дерева
	...
	{
		if (leaf == false)
			FileIndex->ReadNode(this->childrens[i])->Traverse();
		cout << " " << keys[i];
	}
	if (leaf == false)
		FileIndex->ReadNode(this->childrens[i])->Traverse();
...
{
	Node* newChildren = new Node(order);
	newChildren->leaf = root->leaf;
...
void Node::Insert(int k)  //Добавление узла
{
	...
	{
		for (; i >= 0 && keys[i] > k; i--);
		Node* tmp = FileIndex->ReadNode(this->childrens[i + 1]);
		if (tmp->keys_count == 2 * order - 1)
		{
			Split(i + 1, tmp);
			if (keys[i + 1] < k)
				i++;
